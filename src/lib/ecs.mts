/*
(small, 2kb, MIT) -- using this 
https://github.com/jprochazk/uecs

(has networking, up to date MIT)
...
class World {
    ...
    static generateView(world:World,types:Array<Constructor<Component>>){
        const stores:Array<ComponentStorage<Component>> = [];
        for(const t of types){
            stores.push(world.components[t.name]);
        }
        return function(callback:any){
            const retVals = Array(stores.length);
            for (const entity of world.entities.values()) {
                let found = true;
                for(let i=0;i<stores.length;i+=1){
                    const component = stores[i];
                ...
                if(!found){continue;}
                if (callback(entity,...retVals) === false) return;
            }
 ...
 * ```
 */
interface View<T extends Constructor<Component>[]> {
    /**
...
class ViewImpl<T extends Constructor<Component>[]> {
    ...
    constructor(world: World, types: T) {
        this.view = World.generateView(world, types);
    }
